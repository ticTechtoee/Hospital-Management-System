...
Imports System.IO
Public Class PrintMedicalCertificate
   ...
   Select txt.Name
        ...
        Else
            ...
            Dim pdfWrite As PdfWriter = PdfWriter.GetInstance(pdfDoc, New FileStream(txtID.Text & " " & txtName.Text & " Certificate.pdf", FileMode.Create))
            pdfDoc.Open()
            ...
            pdfDoc.Add(Chunk.NEWLINE)
            pdfDoc.Add(Chunk.NEWLINE)
            pdfDoc.Add(New Paragraph(" " & txtRemarks.Text))
            pdfDoc.Add(Chunk.NEWLINE)
            pdfDoc.Add(Chunk.NEWLINE)
            pdfDoc.Add(Chunk.NEWLINE)
            pdfDoc.Add(Chunk.NEWLINE)
            pdfDoc.Add(Chunk.NEWLINE)
            pdfDoc.Add(Chunk.NEWLINE)
            pdfDoc.Add(New Paragraph(" " & txtFD.Text))
            pdfDoc.Close()
        End If
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        Else
            ...
            Try
               ...
               `Date_Of_Consultation`,`doctorFindings`, `finalD`,`Status` FROM `patient_info` 
               WHERE ID = '" & txtSearch.Text & "' UNION SELECT `ID`, `Name`, `sex`, `address`, `age`, `Nationality.`, `dateofadmission`,
              `dateofdischarge`,  `Date_Of_Consultation`,`doctorFindings`, `finalD`,`Status` FROM `patient_infom` WHERE ID = '" & txtSearch.Text & "'"
                ...
                While reader.Read
                    ...
                    txtFD.Text = reader.GetString("doctorFindings")
                    txtRemarks.Text = reader.GetString("finalD")
